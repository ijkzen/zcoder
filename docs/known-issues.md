...
> 2026-08-20 更新——问题 1、2 均已实施并经真机验证（随 c19c901 / v0.5.0 落地）；问题 3 在两者修复后
> 未再复现，状态改为「待复现」，暂不实施候选方案。
...
**真机验证（2026-08-20）**：后台触发断连 → 通知弹出后一直保留到连接真正恢复（`recoveredStream`）或
用户主动断开；不再出现 931ms 即被撤销。次级 bug（transport-fault 路径恢复后通知残留）一并复测通过。
...
## 问题 2：App 一进后台就稳定断连 ★已修复（2026-08-19 实施，2026-08-20 真机验证，c19c901）
...
**实施（c19c901，随 v0.5.0 发布）**：
- `AndroidManifest.xml` 申请 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`。
- `main.dart` 启动时检测电池优化是否已豁免；未豁免则弹提示引导授权。
- `settings_page.dart` 新增「后台保活」段：电池优化状态 + 厂商适配指引（小米/华为/OPPO/vivo/三星分步）。
- `MainActivity.kt` 新增 `isIgnoringBatteryOptimizations` / `getManufacturer` / `openBatteryOptimizationSettings`；
  `battery_optimization.dart` 为 Dart 封装（遵循 `app_version.dart` 模式）。

**真机验证（2026-08-20）**：按「快速验证法」复测——授权电池「不受限制」并据厂商指引开启自启动/后台后，
锁屏/切后台不再稳定断连，回前台无需重新连接，即实锤系统层省电机制为原根因。

## 问题 3：唯一手机控制端却被通知「被其他设备接管」（自抢线误判）★待复现

**症状**：自始至终只有本手机远程连接，断线后系统通知却显示「已被其他设备接管」。

**状态（2026-08-20）**：问题 1、2 修复后（v0.5.0）未再复现。其根因链条依赖「后台断连→回前台重连」
前提，问题 2 修复（不再后台断连）很可能已顺带消除本问题的触发条件；先标记「待复现」持续观察，
在复现确认前**不实施**下述候选方案。

**根因（强假设，尚未实锤）**：中继强制**同一配对同一时刻仅一台终端**（
...
> 问题 1、2 已通过文件日志确认根因并经真机验证闭环（2026-08-20）。
> 问题 3 在问题 1、2 修复后未再复现，已改为「待复现」（见「问题 3」一节）——复现确认前不实施修复；
> 若再次出现，按下方埋点关键字对照时间线（socket 关闭时刻 vs KICKED 到达时刻）定论后再动手。
